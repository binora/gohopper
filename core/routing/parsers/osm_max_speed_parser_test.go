...
func TestParseMaxspeedString(t *testing.T) {
	assert.InDelta(t, 40, ParseMaxspeedString("40 km/h", nil), 0.1)
	assert.InDelta(t, 40, ParseMaxspeedString("40km/h", nil), 0.1)
	assert.InDelta(t, 40, ParseMaxspeedString("40kmh", nil), 0.1)
	assert.InDelta(t, 64.4, ParseMaxspeedString("40mph", nil), 0.1)
	assert.InDelta(t, 48.3, ParseMaxspeedString("30 mph", nil), 0.1)
	assert.InDelta(t, 18.5, ParseMaxspeedString("10 knots", nil), 0.1)
	assert.InDelta(t, 19, ParseMaxspeedString("19 kph", nil), 0.1)
	assert.InDelta(t, 19, ParseMaxspeedString("19kph", nil), 0.1)
	assert.InDelta(t, 100, ParseMaxspeedString("100", nil), 0.1)
	assert.InDelta(t, 100.5, ParseMaxspeedString("100.5", nil), 0.1)
	assert.InDelta(t, 4.8, ParseMaxspeedString("3 mph", nil), 0.1)

	assert.InDelta(t, MaxSpeedNone, ParseMaxspeedString("none", nil), 0.1)
}

func TestParseMaxspeedStringInvalid(t *testing.T) {
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("-20", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("0", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("1", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("1km/h", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("1mph", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("2", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("3", nil))
	assert.Equal(t, ev.MaxSpeedMissing, ParseMaxspeedString("4", nil))
}
